...
#-------------- Create masks from HSV values
    cal_lower_1 = np.array([76,151,0])
    cal_upper_1 = np.array([93,0,119])
    cal_mask1 = cv.inRange(hsv, cal_lower_1, cal_upper_1)
    # mask1 = ~mask1
    cv.imshow('mask1', cal_mask1)
    ...
    for cnt in contours:
            ...
            cv.waitKey(0)
    cv.putText(img, "surface area of mandarine = " + "{}".format(area), (30, 700), cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 2)            
